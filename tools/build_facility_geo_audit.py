...
**このスクリプト自体は参照側の座標を採用しない。** 出力は監査結果(距離と分類)だけで、
`facility_geo_linkage.csv` の座標は書き換えない。医療情報ネットを座標源として統合
するかどうかは2026-08-05(M13)に決定した(一部採用)。採用の実施はこのスクリプトの外、
`tools/build_web_facilities.py`(表示用データセットの生成)で行う。判断材料と決定の
記録は `doc/DECISION_FACILITY_COORDINATES.md` にある(`doc/REQUIREMENTS.md` §4.3
「位置の推測はしない」との関係を含む)。
...
def build_report_markdown(rows, reference_stats, checks) -> str:
    ...
    w("")
    w(
        "可視化サイトが地図に出している医療機関の座標のうち大半は、`ksj/P04-20`（国土数値情報 "
        "医療機関データ、**令和2年度**）との名寄せで与えている。原典の医療機関個票"
        "（`R7/001723127.xlsx`）は**令和7年度**公表であり、5年の開きがある。名寄せは名称の"
        "同一性しか見ていないため、**その間に移転した施設には旧所在地の座標が付いたまま**に"
        "なりうる。本レポートが検算するのは、このP04由来の座標である（2026-08-05・M13以降、"
        "P04で位置が定まらなかった施設には医療情報ネットの公表座標を採用しているが、それは"
        "参照側そのものなので検算の対象にならない）。"
    )
    w("")
    w(
    ...
    w("")
    w(
        "**このスクリプト自体は参照側の座標を採用しない。** `facility_geo_linkage.csv` は書き換えず、"
        "監査結果だけを出力する。医療情報ネットを座標源として採用するかどうかは2026-08-05（M13）に"
        "決定し（一部採用）、その採用は下流の `tools/build_web_facilities.py`（表示用データセットの"
        "生成）が行う。判断材料と決定は `doc/DECISION_FACILITY_COORDINATES.md` にある。"
    )
    w("")
    w("## 2. 参照データ")
    w("")

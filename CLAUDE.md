...
| パス | 内容 |
|---|---|
| iryojoho/01-1_hospital_facility_info_20250601.zip | 病院（施設票）7,587件・2025-06-01時点。コミット済み |
...
- **`ID` は恒久IDではない**（出典ページが「移転等でも同じIDのままの場合がある」と明記）。実測では82,841件中の重複1組のみでほぼ一意だが、年度・制度をまたぐ同一性の保証には使えない。
- 公表時点は5つある（2024-08-01〜2026-06-01）。**R7個票と同じ年度内の 2025-06-01 を採る**（最新版だと「原典が誤っている」のか「原典の後に移転した」のかを分離できない）。
...
**CSVを1本増やすと `web/scripts/lib/bundle.mjs` の `BUNDLE_CSV_FILES` への追加が必須**（`sync-data` がこの配列と `data/processed/*.csv` の実在一覧を突合してビルドを落とす）。加えて**「CSV○本」「○エントリ」と件数を書いた記述がコード・コメント・テスト・ドキュメントに散っている**ので、`grep -rn '<旧件数>本'` で全て直すこと（M7で13→15本にしたとき、`bundle.mjs`・`bundle.test.ts`・`sync-data.mjs` のコメント・`types.ts` のJSDoc・`CLAUDE.md`・`README.md` の6か所に出現した。M10で16→17本にしたときも同じ6か所＋`bundle.mjs` のZIP同梱README本文の「15本」表記）。
...
40. **章を折りたためるようにすると、画面の文言にある「上の」「下の」という位置参照が静かに壊れる**（M14）: サイドパネルをアコーディオン化した瞬間、`FlowPanel` の「※ **上の**「推計流出患者割合」…とは別の統計です」が指す先は「別の章、しかも畳まれていることがある」ものになった。**位置語をやめて章の名前で指すこと**（「「基礎情報」の「推計流出患者割合」…」）。同様に、出典欄の中の入れ子の折りたたみを指す「下の「データの既知の問題」」は、到達経路をそのまま書く（「「出典・注記」の「患者の流入・流出」→「データの既知の問題」」）。**`tsc`・vitest・`vite build` はどれも検出しない**（罠15・22・27・29と同じく、文章として読んで初めて分かる類）。新しい折りたたみを足すときは `grep -n '上の\|上記\|下の\|下記' web/src/components/*.tsx` で地の文の位置参照を洗うこと。
41. **階層（level）に依存しない注記を、階層に連動するブロックの中へ入れない**（M14）: 出典欄を「表示中の階層の可視化の出典だけ出す」ようにした際、地図の初期表示範囲の注記（小笠原諸島・南鳥島が範囲外）が病床＝区域層のブロックの中にあったため、都道府県表示にすると**同じ地図・同じ表示範囲なのに注記だけ消える**という退行になった。注記を書くときは「どのデータセットの話か」と「どの可視化の話か」を分け、後者のうち階層に依らないものは階層ゲートの外へ出す。
42. **要件§6「すべての可視化に出典を表示」は、出典を全部並べることではなく「その可視化が出ている間は必ず出典も出ている」ことで満たす**（M14）: 出ていない可視化の出典まで常時並べると、利用者にとっては探索コストが増えるだけで§6の要求ではない。既に `flowMetadata` の遅延取得で同じ規律を採っていた（罠28）ものを、階層（`level`）と医療機関（区域選択の有無）へ広げた。**逆向きの制約もある**: 要件が明示的に常時表示を求めているのは「病床数と必要量の併記時の比較上の注意書き」だけなので、これは畳まず、併記している表のすぐ下に置く。
43. **`<summary>` を flex で組むときに実機でだけ壊れる2点**（M14。どちらも `tsc`・vitest・`vite build` を素通りし、スクリーンショットを見て初めて分かった）: (a) **`<summary>` の `list-style-position` の既定は `inside`**（HTML標準の推奨レンダリングが `list-style: disclosure-closed inside` を指定している）。内側の要素を `display: flex`（＝ブロックレベル）にすると、**開閉三角だけが1行を占めてラベルが次の行へ落ちる**（実測で summary の高さが 26px→47px）。`list-style-position: outside` を明示すること。(b) **`.side-panel h3` の打ち消しは詳細度で負ける**: `.panel-section-summary-inner h3` は `.side-panel h3` と同じ詳細度 (0,1,1) なので、後に書かれている方が勝つ。`border-bottom: none` が効かず、**見出しの文字幅ぶんの下線と summary 行の全幅の罫線が二重に出る**。セレクタに `.side-panel` を足して (0,2,1) にする（並び順に依存する解決は次の編集で再発する）。
44. **注記の警告色は「原典側の欠陥」だけに使う**（M14）: `.caveat` の琥珀ボックスが最大17個並んでいたため、本当に警告すべきもの（三重県の合成境界・`known_issues`）が埋もれていた。3段に分ける ―― **ティア1（琥珀）＝原典側の欠陥・派生物である警告・§6必須注記／ティア2（グレーボックス `#f2f1ec`）＝判断を要する注記／ティア3（枠なしの小さい ※）＝定義・読み方**。畳んだブロックの中の注記は、そこに注意事項があること自体が見えなくなるので、`<summary>` に件数バッジ（`.source-block-badge`）を出して補う。**新しい色は増やさない**（既存パレットの使い分けだけで表現する）。
...
- **①表示条件絞り込みCSV**（`web/src/lib/downloads.ts` の関数群、`web/src/lib/csv.ts` の `toCsvText` でシリアライズ）:
  - `buildAreaTableCsv` — 地図に表示中の指標（病床機能1つ or 需要区分1つ×年度1つ）を全339区域ぶん（Controls「表示中のデータをCSV」、構想区域表示中）
  ...
  - 各関数とも由来ヘッダー（`#`行、出典・出力条件・注記）をCSV本文の先頭に埋め込む（`buildPreamble`）。ダウンロード実行は `triggerDownload.ts` に分離
  - （M7で `buildAreaFlowCsv`、M11で都道府県層の2本を追加。下記参照）
- **②加工済みデータ一括DL**: `web/scripts/sync-data.mjs` が `data/processed/` の加工済みCSV17本＋各`.meta.json`をZIP化し `web/public/downloads/chiiki-iryo-koso_processed-csv_R6_R7.zip`（36エントリ・約3.4MB）として書き出す。ZIP本体の組み立ては依存ゼロの自前実装（`web/scripts/lib/zip.mjs`）、MANIFEST.tsv・README.mdの内容は `web/scripts/lib/bundle.mjs`。`web/public/downloads/area_boundaries_R7.geojson`（正本の単体コピー）も同時に書き出す。画面側は `BulkDownload.tsx` が `download_manifest.json` を表示するだけで、ZIP自体はブラウザの通常のダウンロードに任せる（CSV本数・エントリ数・ファイル名はその後のM7・M9で上記の通り更新された）
...
- 配信は `web/public/flow/area_flow.json`（バンドルせず、区域を初めて選んだときに1回だけfetch。罠27）。加工済みCSV一括ZIPは13本→15本（32エントリ）になった（その後M9で16本・34エントリ、M10で17本・36エントリへ更新）
...
- **突合の結論**（`tools/verify_yoy_R6_R7.py` → `doc/YOY_VERIFICATION.md`）: 339区域のコード・名称は完全一致。2015〜2023実績と2025必要数は全1695セル一致。**2024実績だけ1281/1695セルで不一致で、R6側が健全**（R6の区域別2024実績は都道府県版と235/235キーで一致、R7は230/235で不一致）。**都道府県レベルは、都道府県別の2024年実績自体がR6公表分とR7公表分で完全一致するため対象外と判断した**（見込量比・実績の1年変化の2指標はいずれも2024年実績どうしの比較ではなく都道府県層でも成立するため、この判断はM12で覆り都道府県層でも実装した。下記M12節参照）
...
**M11「都道府県ぶんの絞り込みCSV」完了**: M6で区域層にしか無かった①表示条件絞り込みCSVを、都道府県層（概観レイヤ）にも用意した。新しいデータセットは作っていない（既存の `prefecture_indicators_R7.json` だけを読む）。
- `buildPrefectureTableCsv` — 地図に表示中の指標を全47都道府県ぶん（Controls「表示中のデータをCSV（47都道府県）」）。`App.tsx` が `level` で `buildAreaTableCsv` と出し分ける
- `buildPrefectureDetailCsv` — 選択中の都道府県1つの基礎情報・病床・医療需要推計をlong形式で（PrefecturePanel「この都道府県の指標をCSV」）
- **全国（`pref_code='00'`）は行に含めない**。47都道府県と同じ列に並べると、利用者が合計したとき二重計上になる（画面のパネルは「（参考）」と畳んで区別しているが、CSVには畳む仕組みが無い）。除外した理由は出力条件行に必ず書く
- **行の由来が混在するので note 列で区別する**: 病床・2020年人口・面積は厚労省の都道府県別公表値そのもの、需要と基準人口/2040年人口は構想区域からの合計（罠25の「値の真横にも注記」をCSVで満たすには列を使う）。preamble側の文言は `known_issues` の `prefecture_demand_aggregated_by_this_repository` から引き、ハードコードしない
- 出典は病床側 `source_beds` と需要側 `source_demand` を使い分ける（都道府県のmetadataには単一の `source` キーが無い。罠11）。detail CSVは両方含むため病床側を主・需要側を追加の出典行にする
- **M11時点では都道府県×年度間比較（R6→R7）だけは出せなかった**。`buildPrefectureTableCsv` は yoy 指標で throw し、`Controls` がその組み合わせでボタンを無効化して理由を `title` に出していた（M12で解除。`buildPrefectureTableCsv` は現在 yoy 指標で `buildYoyPrefectureTableCsv` へ分岐する）

**M10「医療機関座標の監査 + カバレッジ表示」完了**: 要件 `doc/REQUIREMENTS.md` §7 の未決事項（住所ジオコーディングへの切替）に決着をつけた。
- **§7 は No-Go で決着**（決定記録は `doc/DECISION_FACILITY_COORDINATES.md`）。原典が住所（番地）を持たずジオコーディングの入力自体が別データセット依存になるため名寄せ問題が移動するだけであること、および §4.3「位置の推測はしない」に反することが理由。**代わりに医療情報ネットが公表座標そのものを持つことが判明**したが、その統合は §4.3 の改定を伴うため別件として保留（実測では830件を回収でき 87.1%→94.2% になる）
- 監査 `tools/build_facility_geo_audit.py` → `data/processed/facility_geo_audit.csv`（11,760行）・`doc/FACILITY_GEO_AUDIT.md`（検証10項目）。**参照側の座標は採用しない**（`facility_geo_linkage.csv` を書き換えない。罠37）
- **結果: 距離の中央値13m・90%点65m・99%点977mで、P04由来の座標は大半が今も妥当。ただし76件が1km以上（最大32km）食い違う**。住所を突き合わせると「住所は実質同一なのに座標だけ数km離れている（＝いずれかの座標そのものが誤り）」と「住所自体が違う（＝移転）」の2種類がある
- この76件は**表示用データセットで座標を出さない**（`coordinate_withdrawn`。一覧には21指標とともに残す）。地図に出る座標は 10,244→**10,168件**。原典側の欠陥として `known_issues` に登録（`facility_coordinate_conflicts_with_published_reference`）
- 区域パネルに座標カバレッジを常設（例「地図に表示：46件 / この区域の50件（名寄せで位置を特定できず 3件、座標が不一致 1件）」）。凡例にも「点の密度で地域を比べることはできない」旨を明記
...
**M14「サイドパネルの情報設計」完了**: 「情報量が多く、留意点が数値の途中に大量に挟まって読みづらい」という指摘への対応。**新しいデータは一切扱っていない**（`tools/`・`data/processed/`・`web/scripts/` は無変更で、`web/src/` のJSXとCSSだけ）。**画面から読める情報は1件も減らしていない** ―― 常時表示の強制を解いて折りたたみ・脚注化へ再配分しただけ。
- 共通コンポーネント `web/src/components/PanelSection.tsx`（新規）: `<section aria-label>` の中に制御パターンの `<details open={open} onToggle={…}>` を持つ章。**`<summary>` の中に操作要素（ボタン）を置かない設計**にしてある（クリックが開閉と二重に発火するため。CSVボタン類は本文の先頭 `.panel-section-actions` へ移した）
- 区域パネル6章（病床／基礎情報／医療需要推計＝既定で開く、年度間比較／患者の流入・流出／医療機関＝既定で閉じる）・都道府県パネル4章・`BulkDownload` を章立て。**`PanelSection` は `FlowPanel`・`FacilityList` の外側に置く**（`FacilityList` は行展開状態のリセットのため `key={area_code}` で作り直されるので、内側に置くと章の開閉状態まで区域切替で飛ぶ）
- **畳んでも失われてはいけない情報は `<summary>` へ逃がす**: 医療機関の章は畳んだままでも件数と座標カバレッジ（`facilityCoverageSummary()`、M10で常設と決めた情報）が出る。出典ブロックは `known_issues` の件数バッジが出る
- 出典欄（`SourceNotes`）を**表示中の階層に連動**させ、データセットごとに折りたたんだ（罠41・42）。注記の警告色を3段に整理した（罠43）
- 報告率2行を区域パネルの「基礎情報」から「年度間比較」の章へ移した（`PrefecturePanel` と置き場所が食い違っていたのを解消。注記の「報告率が年により異なるため（上記参照）」が同じ章の中を指すようにもなる）
- **トレードオフ**: 折りたたんだ中身はブラウザの Ctrl+F で見つからない。既に `known_issues`・収録CSV一覧で採っていた方式の延長だが、章単位に広げたぶん影響は大きい
- 実装で判明した罠は「可視化実装で判明した罠」節の40〜44に記録

...
43. **注記の警告色は「原典側の欠陥」だけに使う**（M14）: `.caveat` の琥珀ボックスが最大17個並んでいたため、本当に警告すべきもの（三重県の合成境界・`known_issues`）が埋もれていた。3段に分ける ―― **ティア1（琥珀）＝原典側の欠陥・派生物である警告・§6必須注記／ティア2（グレーボックス `#f2f1ec`）＝判断を要する注記／ティア3（枠なしの小さい ※）＝定義・読み方**。畳んだブロックの中の注記は、そこに注意事項があること自体が見えなくなるので、`<summary>` に件数バッジ（`.source-block-badge`）を出して補う。**新しい色は増やさない**（既存パレットの使い分けだけで表現する）。
...
- 実装で判明した罠は「可視化実装で判明した罠」節の40〜43に記録
